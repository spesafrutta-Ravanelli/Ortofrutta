        ...
        <div class="products-grid">
          ...
          >
            <div class="product-image">
              ...
              />
              <div v-if="product.stock <= 5 && product.stock > 0" class="stock-badge low">⚠️ Scorte Basse</div>
              <div v-if="product.stock === 0" class="stock-badge out">❌ Esaurito</div>
            </div>
              ...
              <template v-else>
                <div class="admin-edit-form">
                  <div class="edit-group">
                    <label>Nome:</label>
                    <input type="text" v-model="product.name" class="edit-input" />
                  </div>
                  <div class="edit-group">
                    <label>Prezzo (€):</label>
                    <input type="number" v-model.number="product.price" step="0.01" min="0" class="edit-input" />
                  </div>
                  <div class="edit-group">
                    <label>Stock:</label>
                    <input type="number" v-model.number="product.stock" min="0" class="edit-input" />
                  </div>
                  <div class="edit-group">
                    <label>Categoria:</label>
                    <input type="text" v-model="product.category" class="edit-input" />
                  </div>
                  <div class="edit-group full-width">
                    <label>Descrizione:</label>
                    <textarea v-model="product.description" class="edit-textarea" rows="2"></textarea>
                  </div>
                  <div class="edit-group full-width">
                    <label>URL Immagine:</label>
                    <input type="text" v-model="product.imageUrl" class="edit-input" />
                  </div>
                </div>
                <div class="product-actions">
                  <button @click="saveProduct(index)" class="btn-action btn-save">💾 Salva</button>
                  <button @click="deleteProduct(index)" class="btn-action btn-delete">🗑️ Elimina</button>
                </div>
              </template>
            </div>
          </div>
    ...
    <div v-if="showDeleteModal" class="modal-overlay" @click="cancelDelete">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-actions">
          <button @click="confirmDelete" class="btn btn-delete-modal">🗑️ Elimina</button>
          <button @click="cancelDelete" class="btn btn-cancel-modal">❌ Annulla</button>
        </div>
      </div>
    </div>
  </div>
...
//   - Ctrl+Shift+A da tastiera
//   - triplo click sulla mela in Navbar
// NON ridefinire isAdminMode qui, altrimenti si disallinea dal resto del sito.
...
const newProduct = ref({
  name: '', price: 0, description: '', imageUrl: '', stock: 0, category: ''
})
...
const loadProducts = () => {
  try {
    ...
    const raw = saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(demoProducts))
    const list = deepMigrateImageUrls(raw)
    products.value = list
    if (!saved) saveProducts()
    else if (JSON.stringify(raw) !== JSON.stringify(list)) saveProducts()
  } catch {
...
const validateProduct = (p) => {
  if (!p.name?.trim())     return 'Il nome del prodotto è obbligatorio'
  if (p.price < 0)         return 'Il prezzo deve essere un numero positivo'
  if (p.stock < 0)         return 'Lo stock deve essere un numero positivo'
  if (!p.category?.trim()) return 'La categoria è obbligatoria'
  return null
}

const addProduct = () => {
  const err = validateProduct(newProduct.value)
...
const resetForm = () => {
  newProduct.value = { name: '', price: 0, description: '', imageUrl: '', stock: 0, category: '' }
}
...
onMounted(() => {
  loadProducts()
  // NON aggiungere listener tastiera qui: useAdmin.js li gestisce già globalmente
  setTimeout(() => {
...
.mode-toggle {
  ...
  .toggle-btn {
    ...
    &.active .toggle-slider { transform: translateX(30px); }
  }
}
...
.btn-volantino-large {
  ...
  &:hover  { transform: translateY(-3px); box-shadow: 0 6px 25px rgba(0,0,0,0.3); background: #f0f0f0; }
  &:active { transform: translateY(-1px); }
}

.volantino-hint { color: white; margin-top: 1rem; font-size: 0.95rem; }
...
.add-product-form {
  ...
  h2 { color: #2196F3; margin-bottom: 1.5rem; font-size: 1.8rem; }
...
.products-section {
  ...
  .section-header {
    ...
    .admin-badge { background: #2196F3; color: white; padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; }
  }
}
...
.product-card {
  ...
  &.out-of-stock  { border: 2px solid #f44336; opacity: 0.8; }
  ...
  .product-content {
    padding: 1.5rem;
    .product-name        { font-size: 1.4rem; font-weight: 700; color: #333; margin-bottom: 0.5rem; }
    .product-category    { color: #666; font-size: 0.9rem; margin-bottom: 1rem; }
    .product-description { color: #555; line-height: 1.6; margin-bottom: 1rem; min-height: 3rem; }
    .product-footer {
      display: flex; justify-content: space-between; align-items: center;
      margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #e0e0e0;
      .product-price { font-size: 1.5rem; font-weight: 700; color: #4caf50; }
      .product-stock { color: #666; font-size: 0.9rem; }
    }
  }
}
...
.admin-edit-form {
  ...
  .edit-group {
    margin-bottom: 1rem;
    &.full-width { grid-column: 1 / -1; }
    label { display: block; margin-bottom: 0.25rem; font-size: 0.85rem; font-weight: 600; color: #666; }
    .edit-input,
    .edit-textarea {
...
.btn {
  ...
  &.btn-add          { background: #4caf50; color: white; &:hover { background: #45a049; transform: translateY(-2px); } }
  &.btn-reset        { background: #9e9e9e; color: white; &:hover { background: #757575; } }
  &.btn-delete-modal { background: #f44336; color: white; &:hover { background: #d32f2f; } }
  &.btn-cancel-modal { background: #9e9e9e; color: white; &:hover { background: #757575; } }
}

.btn-action {
  padding: 0.5rem 1rem; font-size: 0.9rem; flex: 1;
  border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.3s ease;
  &.btn-save   { background: #2196F3; color: white; &:hover { background: #1976D2; } }
  &.btn-delete { background: #f44336; color: white; &:hover { background: #d32f2f; } }
}
...
@media (max-width: 768px) {
  .admin-header .header-content   { flex-direction: column; text-align: center; }
  .admin-header .header-left      { align-items: center; text-align: center; h1 { font-size: 2rem; } }
  .keyboard-hint                  { flex-direction: column; gap: 0.25rem; font-size: 0.8rem; kbd { font-size: 0.75rem; padding: 0.2rem 0.4rem; } }
  .btn-volantino-large            { font-size: 1.2rem; padding: 1.2rem 2rem; }
  .mode-toggle .mode-label        { font-size: 0.95rem; }
