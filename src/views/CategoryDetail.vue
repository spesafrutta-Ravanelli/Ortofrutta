<template>
  <div class="category-detail">
    <!-- Loading State -->
    <div v-if="!categoria" class="loading">
      ...
      <section class="category-hero">
        <div class="container">
          ...
          <div class="category-header">
            <div class="category-icon-large">{{ categoria.icon }}</div>
            <h1>{{ categoria.nome }}</h1>
            <p class="category-description">{{ categoria.descrizione }}</p>
          </div>
        </div>
      </section>
      ...
      <section class="products-section">
        <div class="container">
          ...
          <div class="products-grid">
            <div 
              v-for="prodotto in productsToShow" 
              :key="prodotto.id"
              class="product-card"
                ...
                <div class="product-overlay" :class="{ 'edit-mode-overlay': isAdminMode && isEditMode }">
                  ...
                  <template v-else>
                    <div class="admin-edit-form">
                      ...
                      <button 
                        @click="removeProduct(prodotto.id)" 
                        class="btn-delete"
        ...
        <div class="container">
          <div class="info-box">
            ...
            <div class="info-content">
              <h3>Disponibilità Tutto l'Anno</h3>
              <p>
                Questi prodotti sono sempre disponibili nei nostri mercati, 
                garantendo qualità e freschezza costante per le tue esigenze quotidiane.
              </p>
            </div>
          </div>
        </div>
...
const enterEditMode = () => {
  if (categoria.value) {
    editableProducts.value = JSON.parse(JSON.stringify(categoria.value.products))
...
const saveChanges = () => {
  ...
  if (categoria.value && editableProducts.value.length > 0) {
    // Aggiungi categoryId a ogni prodotto per l'aggiornamento
    const productsWithCategory = editableProducts.value.map(prod => ({
      ...prod,
      categoryId: categoria.value.id
    }))
    
    // Aggiorna i prodotti nella categoria locale
    categoria.value.products = JSON.parse(JSON.stringify(editableProducts.value))
    
    // Aggiorna lo store usando il metodo esistente
    store.updateProductsFromListino(productsWithCategory)
...
const removeProduct = (productId) => {
  if (confirm('Eliminare questo prodotto?')) {
    const index = editableProducts.value.findIndex(p => p.id === productId)
    if (index !== -1) {
...
.product-image-wrapper {
  ...
  /* In edit mode la card si espande per contenere il form */
  &.edit-mode-active {
    padding-top: 0;
    height: auto;
    background: #2c5f2d;
