  ...
  <div class="eventi-page">
    ...
    <section class="eventi-hero">
      <div class="container">
        ...
        <p class="eventi-subtitle">Sagre, mercati, promozioni speciali e novità del negozio</p>
      </div>
    </section>

    <!-- ── Form Aggiungi Evento (solo Admin) ────────────────────────── -->
    ...
    <section class="eventi-lista">
      <div class="container">
        ...
        <div v-else-if="eventi.length > 0" class="eventi-grid">
          <div v-for="evento in eventi" :key="evento.id" class="evento-card">
            <div class="evento-img-wrapper" @click="apriLightbox(evento)">
            ...
            <div class="evento-info">
              ...
              <div v-if="isAdminMode" class="admin-actions">
                <button class="btn-modifica" @click="apriModifica(evento)">✏️ Modifica</button>
                <button class="btn-elimina" @click="confermaElimina(evento)">🗑️ Rimuovi</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <!-- ── Modal MODIFICA ────────────────────────────────────────────── -->
    <Teleport to="body">
      <div v-if="showModificaModal" class="modal-overlay" @click="chiudiModifica">
        <div class="modal-content modal-content--large" @click.stop>
          <h3>✏️ Modifica Evento</h3>

          <!-- Immagine cliccabile per cambiarla -->
          <p class="modifica-img-label">Immagine — clicca per cambiarla</p>
          <div class="modifica-img-preview" @click="triggerModificaImg">
            <img :src="modificaPreviewUrl || eventoInModifica?.imageUrl" :alt="eventoInModifica?.titolo" @error="handleImgError" />
            <div class="modifica-img-overlay">📷 Cambia immagine</div>
          </div>
          <input ref="modificaImgRef"    type="file" accept="image/*"                    class="file-input-hidden" @change="onModificaImgSelected" />
          <input ref="modificaCameraRef" type="file" accept="image/*" capture="environment" class="file-input-hidden" @change="onModificaImgSelected" />
          <div class="camera-buttons" style="margin-bottom: 1rem">
            <button class="btn-camera" @click="triggerModificaImg">🖼️ Galleria</button>
            <button class="btn-camera" @click="triggerModificaCamera">📷 Fotocamera</button>
          </div>
          <p v-if="modificaNewImg" class="nuova-img-hint">✅ Nuova immagine selezionata</p>

          <div class="form-group">
            <label>Titolo *</label>
            <input v-model="formModifica.titolo" type="text" class="form-input" maxlength="80" />
          </div>
          <div class="form-group">
            <label>Data</label>
            <input v-model="formModifica.data" type="date" class="form-input" />
          </div>
          <div class="form-group">
            <label>Descrizione</label>
            <textarea v-model="formModifica.descrizione" class="form-textarea" rows="3" maxlength="300"></textarea>
          </div>

          <p v-if="modificaError" class="error-msg">⚠️ {{ modificaError }}</p>

          <div class="modal-actions">
            <button class="btn-confirm-modifica" @click="salvaModifica" :disabled="modificaSaving || !formModifica.titolo">
              <span v-if="modificaSaving">⏳ Salvo...</span>
              <span v-else>💾 Salva</span>
            </button>
            <button class="btn-cancel" @click="chiudiModifica">Annulla</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const { uploading, progress, errorMsg, modificaSaving, aggiungiEvento, modificaEvento, eliminaEvento } = useEventiAdmin()
...
const salvaEvento = async () => {
  ...
  const ok = await aggiungiEvento({ file: selectedFile.value, ...form.value })
  if (ok) {
...
const chiudiLightbox = () => { lightboxEvento.value = null; document.body.style.overflow = '' }
const prevEvento = () => { if (!hasPrev.value) return; lightboxIndex.value--; lightboxEvento.value = eventi.value[lightboxIndex.value] }
const nextEvento = () => { if (!hasNext.value) return; lightboxIndex.value++; lightboxEvento.value = eventi.value[lightboxIndex.value] }

// ── Modal MODIFICA ────────────────────────────────────────────────────────
const showModificaModal  = ref(false)
const eventoInModifica   = ref(null)
const modificaError      = ref(null)
const modificaNewImg     = ref(null)       // File nuova immagine (opzionale)
const modificaPreviewUrl = ref(null)       // Anteprima nuova immagine
const modificaImgRef     = ref(null)
const modificaCameraRef  = ref(null)
const formModifica = ref({ titolo: '', data: '', descrizione: '' })

const triggerModificaImg    = () => modificaImgRef.value?.click()
const triggerModificaCamera = () => modificaCameraRef.value?.click()

const onModificaImgSelected = (e) => {
  const file = e.target.files?.[0]
  if (!file) return
  modificaNewImg.value     = file
  modificaPreviewUrl.value = URL.createObjectURL(file)
}

const apriModifica = (evento) => {
  eventoInModifica.value   = evento
  formModifica.value       = { titolo: evento.titolo, data: evento.data, descrizione: evento.descrizione || '' }
  modificaError.value      = null
  modificaNewImg.value     = null
  modificaPreviewUrl.value = null
  showModificaModal.value  = true
  document.body.style.overflow = 'hidden'
}

const chiudiModifica = () => {
  showModificaModal.value  = false
...
const salvaModifica = async () => {
  if (!formModifica.value.titolo || !eventoInModifica.value) return
  modificaError.value = null
  try {
    await modificaEvento(eventoInModifica.value.id, {
      ...formModifica.value,
      nuovaImmagine: modificaNewImg.value || null,
    })
    chiudiModifica()
  } catch (err) {
    modificaError.value = 'Errore durante il salvataggio'
  }
}
...
const confermaElimina = (evento) => { eventoToDelete.value = evento; showDeleteModal.value = true; document.body.style.overflow = 'hidden' }
const annullaElimina  = () => { showDeleteModal.value = false; eventoToDelete.value = null; document.body.style.overflow = '' }
...
// ── Tastiera ──────────────────────────────────────────────────────────────
const onKeydown = (e) => {
  if (e.key === 'Escape') { chiudiLightbox(); chiudiModifica(); annullaElimina() }
...
.upload-area {
  ...
  overflow: hidden; background: #f8faf8; margin-bottom: 0.75rem;
...
.btn-camera {
  ...
  border-radius: 10px; font-size: 0.95rem; font-weight: 600; cursor: pointer; min-height: 44px;
...
.form-input, .form-textarea {
  ...
  font-size: 1rem; font-family: inherit; box-sizing: border-box; transition: border-color 0.2s;
...
.evento-card {
  ...
  &:hover { transform: translateY(-4px); box-shadow: 0 8px 25px rgba(0,0,0,0.13); .evento-img { transform: scale(1.04); } .zoom-hint { opacity: 1; } }
}
.evento-img-wrapper { position: relative; aspect-ratio: 16 / 9; overflow: hidden; background: #f0f0f0; cursor: pointer; }
...
.evento-titolo      { font-size: 1.15rem; font-weight: 700; color: #2c5f2d; margin: 0 0 0.5rem; }
.evento-descrizione { font-size: 0.9rem; color: #666; line-height: 1.5; margin: 0 0 1rem; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }

.admin-actions { display: flex; gap: 0.75rem; margin-top: 0.75rem; }
.btn-modifica {
  flex: 1; padding: 0.6rem 1rem; background: #fff3e0; color: #e65100;
  border: 2px solid #ff9800; border-radius: 8px; font-size: 0.9rem; font-weight: 600;
  cursor: pointer; min-height: 40px; transition: all 0.2s;
  &:hover { background: #ff9800; color: white; }
}
.btn-elimina {
  flex: 1; padding: 0.6rem 1rem; background: #fdecea; color: #c62828;
  border: 2px solid #f44336; border-radius: 8px; font-size: 0.9rem; font-weight: 600;
  cursor: pointer; min-height: 40px; transition: all 0.2s;
...
.evento-card.skeleton {
  ...
  .skeleton-text { padding: 1.1rem 1.25rem; }
...
/* ── LIGHTBOX ── */
.lightbox-overlay {
  position: fixed; inset: 0; z-index: 3000; background: rgba(0,0,0,0.93);
  display: flex; align-items: center; justify-content: center; animation: fadeIn 0.2s ease; padding: 1rem;
}
.lightbox-content { display: flex; flex-direction: column; max-width: 900px; width: 100%; max-height: 90vh; background: #1a1a1a; border-radius: 16px; overflow: hidden; animation: scaleIn 0.25s ease; }
...
.lightbox-nav {
  ...
  background: rgba(255,255,255,0.15); border: none; border-radius: 50%;
  color: white; font-size: 2rem; cursor: pointer; z-index: 3001;
  display: flex; align-items: center; justify-content: center; backdrop-filter: blur(8px);
  &:hover { background: rgba(255,255,255,0.3); }
}
...
/* ── Modal ── */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 2000; padding: 1rem; overflow-y: auto; }
.modal-content {
  background: white; border-radius: 16px; padding: 2rem; max-width: 480px; width: 100%;
  box-shadow: 0 10px 40px rgba(0,0,0,0.25); margin: auto;
  h3 { margin: 0 0 1.25rem; color: #333; font-size: 1.3rem; }
  p  { color: #555; margin: 0.5rem 0; }
  .modal-warning { color: #f44336; font-weight: 600; font-size: 0.9rem; }
}
.modal-content--large { max-width: 560px; }

/* Immagine nel modal modifica */
.modifica-img-label { font-weight: 600; color: #333; font-size: 0.9rem; margin: 0 0 0.35rem; }
.modifica-img-preview {
  position: relative; width: 100%; aspect-ratio: 16/9; border-radius: 10px; overflow: hidden;
  background: #f0f0f0; margin-bottom: 0.75rem; cursor: pointer;
  img { width: 100%; height: 100%; object-fit: cover; transition: filter 0.2s; }
  &:hover img { filter: brightness(0.7); }
  &:hover .modifica-img-overlay { opacity: 1; }
}
.modifica-img-overlay {
  position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
  color: white; font-weight: 700; font-size: 1rem;
  background: rgba(0,0,0,0.4); opacity: 0; transition: opacity 0.2s;
}
.nuova-img-hint { font-size: 0.85rem; color: #2c5f2d; font-weight: 600; margin: 0 0 0.75rem; }

.modal-actions { display: flex; gap: 0.75rem; margin-top: 1.5rem; }
.btn-confirm-modifica {
  flex: 1; padding: 0.875rem; background: #4caf50; color: white;
...
.btn-confirm-delete {
  flex: 1; padding: 0.875rem; background: #f44336; color: white;
  border: none; border-radius: 10px; font-weight: 700; font-size: 1rem; cursor: pointer; min-height: 44px;
}
.btn-cancel {
  flex: 1; padding: 0.875rem; background: #f5f5f5; color: #333;
  border: none; border-radius: 10px; font-weight: 600; font-size: 1rem; cursor: pointer; min-height: 44px;
}
...
@media (max-width: 768px) {
  .eventi-hero { padding: 2rem 0 1.75rem; .eventi-title { font-size: 1.8rem; } }
